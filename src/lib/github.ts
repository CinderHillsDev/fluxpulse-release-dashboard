...
export async function cacheStatus(
  ...
  const cached = await env.CACHE.get('status', { type: 'json' });
  if (cached) return cached as RepoStatus[];

  const fresh = await getRepoStatus(token);
  await env.CACHE.put('status', JSON.stringify(fresh), { expirationTtl: 300 });
  return fresh;
}

export async function cachePRs(env: Env, token: string): Promise<PR[]> {
  const cached = await env.CACHE.get('prs', { type: 'json' });
  if (cached) return cached as PR[];

  const fresh = await getAllPRs(token);
  await env.CACHE.put('prs', JSON.stringify(fresh), { expirationTtl: 120 });
  return fresh;
}

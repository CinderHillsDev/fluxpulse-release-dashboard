...
export const POST: APIRoute = async ({ request }) => {
  if (env?.SESSION) {
    ...
    await Promise.all([
      ...
      env.SESSION.delete('status:v9'), // legacy keys
      env.SESSION.delete('status:v8'),
      env.SESSION.delete('status:v7'),
      env.SESSION.delete('status:v6'),
      env.SESSION.delete('status:v5'),
